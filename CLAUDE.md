...
- **La base D1 porte les exercices, la banque d'outils, les tables de référence** (décision D47, migration `0005`) **et les images** (D56, migration `0007`) : c'est ce que le serveur et le navigateur lisent, et ce que l'éditeur modifie en production. Les JSON et les images du dépôt ne sont plus que la **semence et les données des tests** :
  ...
  - `reference/semence-d1/generer.mjs` et `generer-images.mjs` — les scripts qui ont composé les migrations `0005` et `0007` ; on ne les relance pas (une migration appliquée ne change pas).
...
site/img/outils/   photos des outils (semence de la table images, D56) ; site/img/pictos/operations/ : les pictogrammes (semence aussi)
worker/            le Worker : API /api/… du serveur de correction et de l'éditeur, et /images/<id> (décisions D19, D20, D47, D56)
migrations/        schéma de la base D1, un fichier SQL numéroté par changement ; 0005 sème la banque et les exercices, 0007 les images
reference/         outillage ponctuel : convertisseur des pictogrammes (D29), générateurs des semences 0005 et 0007
