...
### 3.9 Éditeur des exercices (`/prof/editeur`, décisions D47 à D49)

Univers « exercice », en-tête « Éditeur des exercices », pied avec le nom du département sur trois lignes et « TGM-TMI · Les exercices publiés sont ce que voient les étudiants ; le brouillon ne change rien tant qu'il n'est pas publié. ». **Ordinateur d'abord**, utilisable à 390 px pour une retouche rapide : les grilles de champs passent à une colonne, les tableaux défilent dans leur cadre. Rôle **admin seulement** : la connexion (même panneau que `/prof`, note « L'éditeur n'est pas ouvert à la clé de consultation ») refuse la clé de consultation avec le message du serveur. Barre du haut : « admin », lien **Espace professeur**, **Se déconnecter**. Trois onglets : **Exercices**, **Banque d'outils**, **Sauvegarde** ; quitter une page avec des modifications non enregistrées demande confirmation.
...
- **Sauvegarde** (panneau étroit) : le paragraphe qui dit ce que contient l'export et ce que fait l'import ; 1. **Exporter tout en JSON** (fichier `quiz-parametres-coupe-exercices-AAAA-MM-JJ.json`) ; 2. un champ fichier : l'export choisi est validé et résumé (« Exercices ajoutés : … », « Brouillons remplacés : … », « Versions publiées ajoutées : … », « Les séances, les journaux et les attestations ne sont pas touchés. »), ou ses erreurs en rouge, avec ce qui change dans la banque outil par outil (« Banque d'outils — ajoutés : … ; modifiés : … ; inchangés : n. », puis « aucun outil ne disparaît » ou « DISPARAÎTRAIENT : MVLNR (mvlnr), … » avec l'avertissement) ; le bouton rouge **Importer** ne s'active que sans erreur et demande de taper IMPORTER — s'il y a des disparitions, il s'appelle **Importer et remplacer la banque** et exige REMPLACER (D50).
...
- L'éditeur (jalon 7a, §3.9) écrit en production dans la base D1 (D47, D48). Reste au jalon 7b : le téléversement des images, l'édition du gabarit de nomenclature (D24) et des barres (D25), et l'édition versionnée des tables de référence — vitesses de coupe avec `debut_famille` (D27), avances, opérations et leurs pictogrammes, matériaux d'outil, révision (D28). Il est protégé par la **clé d'administration** du serveur (D21, rôle admin de D44), la même que l'espace professeur — jamais dans le code, jamais dans les docs.
