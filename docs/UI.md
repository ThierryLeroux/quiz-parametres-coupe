...
- **Page d'un exercice**, en panneaux : la barre (titre, identifiant, lien étudiant, date du brouillon ; **← Exercices**, **Aperçu du brouillon**, **Enregistrer le brouillon** (bleu), **Publier…** (doré)), le message d'état (« Brouillon enregistré à 13:05 (révision 3) », ou le refus 409 avec **Recharger la page**), la liste des erreurs sans champ ; **Réglages généraux** (titre ; grandeurs évaluées en cases à cocher ; matières d'outil et groupes permis pour tout l'exercice — tout coché = aucune restriction ; proposé à l'accueil) ; **Outils de l'exercice** : un volet repliable par copie (« 2. mvlnr · MVLNR · 3 réussite(s) de suite », et « 1 erreur » en rouge), le formulaire d'outil dedans, puis **Dupliquer dans l'exercice**, **↑ Monter**, **↓ Descendre**, **Retirer** (confirmation) ; sous la liste, **Ajouter depuis la banque** (liste déroulante des outils disponibles) et **Ajouter depuis un autre exercice** (l'exercice, puis l'outil) ; **Versions publiées** : une ligne par version (numéro, date, tables, séances) avec **Aperçu**.
...
- **Aperçu** : un panneau « Dix questions tirées du brouillon » (ou « de la version n ») : tableau N°, Outil (nomenclature composée), Matière d'outil, Matériau usiné, une colonne par grandeur évaluée (les réponses attendues, chasse fixe) ; **Dix autres**, Fermer. Rien n'est enregistré.
