...
- **Sauvegarde** (panneau étroit) : le paragraphe qui dit ce que contient l'export et ce que fait l'import ; 1. **Exporter tout en JSON** (fichier `quiz-parametres-coupe-exercices-AAAA-MM-JJ.json`) ; 2. un champ fichier : l'export choisi est validé et résumé (« Exercices ajoutés : … », « Brouillons remplacés : … », « Versions publiées ajoutées : … », « Les séances, les journaux et les attestations ne sont pas touchés. »), ou ses erreurs en rouge ; le bouton rouge **Importer** ne s'active que sans erreur et demande de taper IMPORTER.
