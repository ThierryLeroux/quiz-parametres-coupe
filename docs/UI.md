...
- **Page d'un exercice**, en panneaux : la barre (titre, identifiant, lien étudiant, date du brouillon ; **← Exercices**, **Aperçu du brouillon**, **Enregistrer le brouillon** (bleu), **Publier…** (doré)), le message d'état (« Brouillon enregistré à 13:05 (révision 3) », ou le refus 409 avec **Recharger la page**), la liste des erreurs sans champ ; **Réglages généraux** (titre ; **une ligne par grandeur avec trois états** — évaluée, fournie, masquée (D52) — en boutons radio ; matières d'outil et groupes permis pour tout l'exercice — tout coché = aucune restriction, chaque case avec sa **pastille de couleur** (lettre ISO sur la couleur de la classe, couleur de la matière d'outil : celles des feuilles, `tokens.css`) et **Tout cocher / Tout décocher** ; proposé à l'accueil) ; **Outils de l'exercice** : un volet repliable par copie (« 2. mvlnr · MVLNR · 3 réussite(s) de suite », et « 1 erreur » en rouge), le formulaire d'outil dedans, puis **Dupliquer dans l'exercice**, **↑ Monter**, **↓ Descendre**, **Retirer** (confirmation) ; sous la liste, **Ajouter depuis la banque** (liste déroulante des outils disponibles) et **Ajouter depuis un autre exercice** (l'exercice, puis l'outil) ; **Versions publiées** : une ligne par version (numéro, date, tables, séances) avec **Aperçu**.
- **Formulaire d'outil** (le même pour une copie et pour la banque) : Nom, Réussites de suite exigées (copie), Opération (liste), **Gabarit de nomenclature en lecture seule** avec, en note, l'exemple composé et les jetons (« Exemple composé : MVLNR - Ø charioté: 1.000" — jetons : IdDia. Se modifie au jalon 7b. »), Note, Photo (liste des images de `site/img/outils/`, avec la photo affichée), facteurs de vitesse et d'avance, RPM max, dents minimum et maximum, limite d'avance (non utilisée), matières d'outil et groupes usinables en cases à cocher, **Dimensions possibles** en zone de texte (une par ligne, « libellé ; valeur »), Barres et rapport (outil à deux diamètres), Identifiant (lecture seule). Chaque champ porte sa note ; **une erreur remplace la note, en rouge, sous le champ en cause**, en continu à la frappe ; le bouton Publier dit « Publier (2 erreurs à corriger) » et reste désactivé.
