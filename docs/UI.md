...
- **Tableau des questions réussies qui comptent** (D41), titré « Questions réussies qui comptent (n) » : N° (rang dans la séance), Outil (le nom tel qu'affiché à l'étudiant), Matière de l'outil (en court : « Acier rapide », « Carbure solide », « Insert de carbure »), Matériau usiné (« P 1 — Acier non allié, Recuit »), une colonne par grandeur évaluée (« Vc (pi/min) », « N (rév/min) »…, les réponses de l'étudiant), Date et heure (« 2026-09-21 13:12:05 », à l'heure du poste). Dans l'ordre chronologique. Lignes d'une seule hauteur, sans repli ; un libellé trop long est tronqué (le titre de la cellule le garde entier, et `/verifier` montre tout). Absent d'une attestation figée avant cette liste.
- **Une page lettre quand ça tient, sinon la suite sur une deuxième page** (et suivantes) : l'en-tête du département, une ligne de rappel « Attestation de réussite — Prénom Nom · matricule · code XXXXX-XXXXX (suite) », le titre « Questions réussies qui comptent (suite) », la suite du tableau avec ses en-têtes. Le QR, le code et le tableau par outil restent en première page ; son titre de liste dit « — suite à la page suivante ». La coupe est un calcul pur (`paginateQuestions`, `PAGE_LAYOUT` de `attestation-data.js`, calibré sur la page lettre mesurée dans Chrome : 460 px libres sur la page 1 pour les lignes d'outils (24 px) et de questions (20 px), 36 questions par page de suite). À l'écran, les pages s'empilent ; à l'impression, une feuille lettre chacune.
...
- Le résultat, dans un second panneau : **vert** « Attestation valide » avec l'enregistrement complet (le même bloc d'informations, le même tableau des opérations et, s'il y en a une, la **liste des questions réussies** que l'attestation — complète, lignes repliables, rien de tronqué : D41) ; **doré** « Attestation annulée » avec la date, le motif (séance remise à zéro par l'enseignant, ou identité corrigée par l'étudiant et nouvelle attestation émise) et l'enregistrement tel qu'il était ; **rouge** « Aucune attestation ne correspond » ; **rouge** « Signature invalide ou contenu modifié ». Le vert, le doré et le rouge sont doublés par le titre (§7).
...
- L'éditeur (jalon 6) : il édite aussi le gabarit de nomenclature des outils (D24), les barres d'un outil à deux diamètres (D25), `debut_famille` (D27), la révision des tables (D28), les images et les pictogrammes. Il est protégé par la **clé d'administration** du serveur (D21), la même que l'espace professeur — jamais dans le code, jamais dans les docs. Son mode de sauvegarde (JSON à télécharger, ou autre) se décide au jalon 6.
