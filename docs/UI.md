...
- L'exercice vient du serveur (D47) : l'accueil montre la **dernière version publiée** (« version 2 ») ; une séance reprise garde la sienne, et l'écran Question est alors composé avec celle-là. Un exercice **archivé** : « Cet exercice n'est plus offert par ton enseignant : aucune nouvelle séance ne peut être commencée, mais une séance déjà commencée se reprend encore. »
...
- photo (`/images/<image>` : l'image de la base que l'outil nomme, D56 ; halo de la même couleur), **nom de l'outil = son gabarit de nomenclature résolu, tel quel** (D24, SPEC §4.6 : « MVLNR - Ø charioté: 2.000" », « Alésoir 0.6250" - 8 lèvres »), rappel du matériau d'outil en petit ;
...
- une case à chasse fixe, 48 px de haut, `inputmode="decimal"` ; les champs fournis par l'exercice sont grisés et marqués « fourni par l'exercice » ; un champ **masqué** (D52) montre « — » à la place de la case, sans valeur ni saisie, marqué « non demandée » — après la correction il reste tel quel ;
...
- La page est **claire, en format lettre, identique à l'écran et à l'impression** (univers « documents », police Carlito) : logo à gauche, **en-tête du département sur trois lignes** à droite (« Techniques de génie mécanique / Technique du génie de la maintenance industrielle / (fiabilité des systèmes de production) »), titre « Attestation de réussite — calcul de paramètres d'usinage », bloc d'informations sur deux colonnes (Exercice, Version de l'exercice — « version 1 » pour une version publiée numérotée, « r0 » tel quel pour une attestation d'avant le jalon 7a (D50) —, Révision des tables — « A2026_r0 » —, Prénom, Nom, Matricule en chasse fixe, Début de l'exercice, Réussite de l'exercice, Questions réussies ; dates `AAAA-MM-JJ HH:MM` à l'heure du poste), et le **QR code à droite**, avec le **code court dessous** (`XXXXX-XXXXX`, chasse fixe).
...
- Rôle admin : le lien **Éditeur des exercices** dans la barre du haut mène à `/prof/editeur` (§3.9).

### 3.9 Éditeur des exercices (`/prof/editeur`, décisions D47 à D49, D56 à D59)

Univers « exercice », en-tête « Éditeur des exercices », pied avec le nom du département sur trois lignes et « TGM-TMI · Les exercices publiés sont ce que voient les étudiants ; le brouillon ne change rien tant qu'il n'est pas publié. ». **Ordinateur d'abord**, utilisable à 390 px pour une retouche rapide : les grilles de champs passent à une colonne, les tableaux défilent dans leur cadre, les onglets passent à la ligne. Rôle **admin seulement** : la connexion (même panneau que `/prof`, note « L'éditeur n'est pas ouvert à la clé de consultation ») refuse la clé de consultation avec le message du serveur. Barre du haut : « admin », lien **Espace professeur**, **Se déconnecter**. Quatre onglets : **Exercices**, **Banque d'outils**, **Images**, **Sauvegarde** ; quitter une page avec des modifications non enregistrées demande confirmation.

- **Exercices** : tableau Rang, Titre (lien), Identifiant (chasse fixe), État (« Jamais publié », « Brouillon modifié », « À jour » en vert, « Archivé » atténué), Dernière version (« v2 · 2026-09-24 13:05 »), Séances (« 3 séances (v2 : 1, v1 : 2) »), À l'accueil, Actions : **Ouvrir**, **↑** et **↓** (Monter / Descendre : l'ordre de la liste est celui de l'accueil des étudiants, D51 ; désactivés en tête et en queue), **Dupliquer** (demande l'identifiant), **Renommer** (le titre du brouillon), **Archiver** (rouge au contour, confirmation) ou **Rétablir**, **Supprimer** (rouge plein, seulement sans séance, confirmation), **Copier le lien étudiant** (presse-papiers, sinon une boîte avec le lien). Sous le tableau, **Nouvel exercice** : identifiant d'URL et titre, « Créer un exercice vide ».
- **Page d'un exercice**, en panneaux : la barre (titre, identifiant, lien étudiant, date du brouillon ; **← Exercices**, **Aperçu du brouillon**, **Enregistrer le brouillon** (bleu), **Publier…** (doré)), le message d'état (« Brouillon enregistré à 13:05 (révision 3) », ou le refus 409 avec **Recharger la page**), la liste des erreurs sans champ ; **Réglages généraux** (titre ; **une ligne par grandeur avec trois états** — évaluée, fournie, masquée (D52) — en boutons radio ; matières d'outil et groupes permis pour tout l'exercice — tout coché = aucune restriction, chaque case avec sa **pastille de couleur** (lettre ISO sur la couleur de la classe, couleur de la matière d'outil : celles des feuilles, `tokens.css`) et **Tout cocher / Tout décocher** ; proposé à l'accueil). Sous les états, en doré, l'**avertissement non bloquant** des grandeurs déductibles (D54) : une ligne par grandeur à trouver qui se lit dans les grandeurs fournies (« Vc se déduit de N fourni : Vc = N × Ø / (4 × facteur Vc), sauf si N est plafonné par la limite RPM. »), rafraîchie à chaque changement ; le bouton Publier n'en tient pas compte ; **Outils de l'exercice** : une ligne par copie, repliée par défaut — case à cocher, **vignette** de la photo (44 px), le triangle qui déplie, « 2. mvlnr · MVLNR · 3 réussite(s) de suite » (et « 1 erreur » en rouge), puis, **sans déplier**, **Dupliquer**, **↑**, **↓** (désactivés en tête et en queue), **Retirer** (rouge au contour, confirmation) ; le formulaire d'outil se déplie sous la ligne. Au-dessus des lignes, **Tout cocher** et **Retirer la sélection** (rouge au contour, désactivé sans sélection, confirmation qui nomme les outils cochés) ; comme tout le reste de la page, ces retraits ne changent que le brouillon. Sous la liste, **Ajouter depuis la banque** (liste déroulante des outils disponibles) et **Ajouter depuis un autre exercice** (l'exercice, puis l'outil) ; **Versions publiées** : une ligne par version (numéro, date, tables, séances) avec **Aperçu**.
- **Formulaire d'outil** (le même pour une copie et pour la banque), **regroupé par thème**, un `fieldset` par groupe avec un intertitre en petites capitales bleues : **Identification** (Nom, Opération, Note affichée sous l'outil, Identifiant en lecture seule, puis **Photo** : la **galerie** (D56) — l'image en cours (aperçu 96 px sur blanc, « nom (identifiant) »), **Choisir une image…** qui déplie la galerie : champ **Galerie** de recherche par nom ou identifiant (sans casse ni accents), champ **Téléverser une image** (fichier), et les vignettes des images « photo d'outil » non archivées (l'archivée que l'outil porte déjà reste visible, marquée), la choisie en surbrillance ; **Aucune** ; une photo téléversée est réduite dans le navigateur (800 px ; JPEG sur fond blanc, ou PNG sans fond si elle a de la transparence, D60) puis choisie d'office, ou l'existante si c'était un doublon — le message le dit) ; **Nomenclature** (le **gabarit éditable** (D58), puis une rangée de boutons **[IdDia] [Dia] [Pas] [IdBarre] [NbDent] [NomOutil] [Operation] [Matoutil]** — seulement ceux qui ont un sens pour l'outil : [Pas] en filetage, [IdBarre] avec des barres — qui insèrent le jeton au curseur ; la note ; puis « Exemple composé : MVLNR - Ø charioté: 1.000" — jetons : IdDia. » suivi du lien **Autre exemple**, qui tire d'autres valeurs dans l'outil ; un jeton inconnu ou sans valeur, ou un crochet non apparié, est l'erreur rouge sous le champ) ; **Dimensions** (**Dimensions possibles** en zone de texte, une par ligne « libellé ; valeur » ; Barres et rapport pour un outil à deux diamètres ; et, pour un outil de filetage, à droite, la liste **« Lecture par le moteur »** : une ligne par dimension, « 1/4- 20 UNC → Ø 0.25 po · pas 0.05 po (20 filets/po) », « M10 x 1.50 → Ø 10 mm = 0.3937 po · pas 1.5 mm = 0.05906 po » en métrique, ou l'erreur de lecture en rouge) ; **Dents** (minimum et maximum côte à côte) ; **Facteurs** (vitesse × Vc, avance) ; **Limites** (RPM max de la machine, Limite d'avance « non utilisée ») ; **Matières et groupes permis** (cases à cocher avec pastilles, Tout cocher / Tout décocher) ; **Exercice** (Réussites de suite exigées — copie seulement). Chaque champ porte sa note ; **une erreur remplace la note, en rouge, sous le champ en cause**, en continu à la frappe ; le bouton Publier dit « Publier (2 erreurs à corriger) » et reste désactivé.
- **Publier** : après l'enregistrement, un panneau doré « Publier la version n de « … » ? » liste les différences avec la version précédente (« Titre : « … » → « … » », « Outil ajouté : Alésoir (alesoir), 1 réussite de suite », « Outil retiré : … », « MVLNR (mvlnr) — fact_vc : « 1 » → « 0.5 » », « L'ordre des outils a changé. »), puis, s'il y a lieu, « Avertissement, sans effet sur la publication : une grandeur à trouver se déduit des grandeurs fournies. » et les mêmes lignes dorées (D54), rappelle que les séances commencées gardent leur version, puis **Publier la version n** / Annuler. Sans différence avec la dernière version, le bouton est désactivé et dit « Aucune différence à publier » (D51). Ensuite la page se recharge avec « Version n publiée le … ».
- **Aperçu** : un panneau « Dix questions tirées du brouillon » (ou « de la version n ») : tableau N°, Outil (nomenclature composée), Matière d'outil, Matériau usiné, puis les cinq grandeurs, chacune avec son état dans l'en-tête (« RPM (N) · évaluée ») : la réponse attendue si évaluée, la valeur si fournie, « — » si masquée (D52) ; **Dix autres**, Fermer. Rien n'est enregistré.
- **Banque d'outils** : tableau Nom (lien), Identifiant, Opération, Dimensions (nombre), Exercices qui en ont une copie (« 2 (m10-tournage-vc, m10-tournage-vc-copie) »), État (disponible, archivé le …), Actions : **Ouvrir**, **Dupliquer**, **Archiver** / **Rétablir** ; « Nouvel outil : identifiant » et **Créer un outil** (un outil de départ à compléter). La fiche d'un outil : le formulaire d'outil, **Enregistrer l'outil**, la mention des exercices qui en ont une copie (« les copies ne suivent pas »).
- **Images** (D56, D57) : le paragraphe qui dit la règle (une image ne change jamais sous le même identifiant ; utilisée par une version publiée, elle s'archive, jamais supprimée) ; filtres **Usage** (toutes, photo d'outil, pictogramme d'opération) et **Recherche** (nom ou identifiant, à la frappe) ; tableau : vignette (44 px sur blanc), **Nom** avec l'identifiant en chasse fixe dessous, Usage, Type (« png · 6.5 Ko »), Ajoutée le, **Utilisée par** (« 1 version publiée · 1 brouillon · 1 outil de la banque », « 1 version des tables », « jamais utilisée »), État (« offerte », « archivée le … », la ligne atténuée), Actions : **Renommer** (le nom lisible), **Archiver** (rouge au contour, confirmation) ou **Rétablir**, et **Supprimer** (rouge plein, confirmation) **seulement pour une image jamais utilisée**. Sous le tableau, « n images sur m. », puis **Téléverser une image** : Usage, Fichier (PNG, JPEG, WebP, GIF, BMP ou SVG ; la note dit la réduction), **Téléverser** ; le message dit le résultat — « « nom » téléversée (identifiant, taille). », « Cette image était déjà dans la base : … », et pour un SVG « Retiré du SVG : … », ou le refus du serveur (« SVG refusé : élément <script> … »).
- **Sauvegarde** (panneau étroit) : le paragraphe qui dit ce que contient l'export (images comprises) et ce que fait l'import ; 1. **Exporter tout en JSON** (fichier `quiz-parametres-coupe-exercices-AAAA-MM-JJ.json`) ; 2. un champ fichier : l'export choisi est validé et résumé (« Images : 48 déjà dans la base ; 1 à envoyer avant l'import (une par requête). », « Exercices ajoutés : … », « Brouillons remplacés : … », « Versions publiées ajoutées : … », « Les séances, les journaux et les attestations ne sont pas touchés. »), ou ses erreurs en rouge, avec ce qui change dans la banque outil par outil (« Banque d'outils — ajoutés : … ; modifiés : … ; inchangés : n. », puis « aucun outil ne disparaît » ou « DISPARAÎTRAIENT : MVLNR (mvlnr), … » avec l'avertissement) ; le bouton rouge **Importer** ne s'active que sans erreur et demande de taper IMPORTER — s'il y a des disparitions, il s'appelle **Importer et remplacer la banque** et exige REMPLACER (D50). Pendant l'import, le message compte les images envoyées (« Envoi de l'image 1 sur 3 : « … »… », D59), puis « Import terminé : … ».
...
Tous en SVG trait/aplat, dans `site/img/pictos/`, référencés par les données ou par le code, et éditables (D11 : l'éditeur permet d'en changer). **État** : les six grandeurs (`grandeurs/`) et les deux miniatures de la feuille Formules (`miniatures/`) sont les SVG des maquettes approuvées, servis comme fichiers ; les dix-neuf opérations (`operations/`) sont **les dessins d'origine du classeur, convertis en SVG sans redessin** (D29) par `reference/pictogrammes-du-classeur/convertir.mjs`, nommés d'après l'opération (`site/img/pictos/README.md`) — depuis le jalon 7b (D56), ils sont **semés dans la base** (migration `0007`, identifiant = le slug de l'opération) et servis par `/images/<id>` ; les fichiers du dépôt sont la semence, et un pictogramme nouveau se téléverse dans l'éditeur (onglet Images, SVG assaini par le serveur, D57).
...
- L'éditeur (jalons 7a et 7b, §3.9) écrit en production dans la base D1 (D47, D48) ; le téléversement des images (D56) et l'édition du gabarit de nomenclature (D58) sont faits. Reste (jalon 7b, partie B) : l'édition versionnée des tables de référence — vitesses de coupe avec `debut_famille` (D27), avances, opérations et leurs pictogrammes, matériaux d'outil, révision (D28) — et l'édition des barres (D25) reste par la zone de texte. Il est protégé par la **clé d'administration** du serveur (D21, rôle admin de D44), la même que l'espace professeur — jamais dans le code, jamais dans les docs.
