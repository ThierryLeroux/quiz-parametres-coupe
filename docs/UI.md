...
**Attestation provisoire** (D29, D30, d'ici le jalon 5) : page lettre claire — en tête, une **bannière rouge encadrée « PROVISOIRE — non signée, ne vaut pas preuve de réussite »**, imprimée aussi ; logo, « Attestation de réussite — calcul de paramètres d'usinage », bloc du département ; bloc d'informations (Exercice avec version, Prénom, Nom, Matricule, Début, Réussite, Questions réussies, Champs évalués) ; à droite, l'**emplacement du code QR** en pointillé (« à venir ») ; le tableau **« Opérations effectuées »** — un rang par outil de l'exercice, dans l'ordre : Outil (nom générique et plage de dimensions, « Foret fractionnaire (Ø 1/64 po à Ø 1 po) »), Opération, Réussites « obtenues / exigées » —, tiré de `progression.outils` que renvoie le serveur ; un avis « **Attestation provisoire, non signée** … » ; pied « TGM-TMI — TLP — 2026 ». Barre : « Enregistre le PDF, puis remets-le sur Léa », **Enregistrer en PDF** (nom proposé `Attestation-<exercice>-<Nom>-<Prenom>`), ← Retour. Le jalon 5 la remplace par la version signée décrite ci-dessous, avec la même liste.
