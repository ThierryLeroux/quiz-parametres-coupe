...
const tablesOf = (row) => ({ materiaux: row.materiaux, operations: row.operations });
...
// Le brouillon d'un exercice, assemblé avec les tables de référence les plus récentes — pour
// l'aperçu de l'éditeur. Jamais gardé en mémoire : il change. Lève si le brouillon est invalide.
//   draft : le brouillon à assembler (celui de la requête, ou celui de la base)
export async function assembleDraft(db, exerciseId, draft) {
  const tables = await base.findLatestTables(db);
  const { exercise, tools } = engineExercise(exerciseId, 'brouillon', draft);
  return { data: assembleData(tablesOf(tables), tools), exercise, version: null, tables_id: tables.id };
}
